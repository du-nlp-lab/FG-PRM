...
from verification.reward_llama3 import FactualityReward
...
selected_generations_file = f'{config["dataset_root"]}selected_generations_llama3_fgprm.json'
reward_raw_score_path = f'{config["dataset_root"]}reward_raw_score_llama3_sc.json'
...
tokenizer = transformers.AutoTokenizer.from_pretrained(
                '/data/models/huggingface-format/llama-3-8b/',
...
for reward_name in reward_model_names:
    ...
    reward_model = FactualityReward(
        tokenizer,
        '../models/llama3_tc_2/',
        f'../models/orm_llama3_{reward_name}',
        0.5,
        -0.5,
    ...
    for k, v in tqdm(generations.items()):
        ...
        for a_idx, answer in enumerate(v):
            ...
            reward_score = torch.prod(raw_rewards[:, -1]).item()
