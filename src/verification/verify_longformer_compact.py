...
from verification.reward import FactualityCompactReward
...
factuality_reward = FactualityCompactReward(
    tokenizer,
    '../models/prm_longformer_compact_ori_test_2',
    0.5,
    -0.5,
    sep = '</s>'
)
...
for k, v in generations.items():
    ...
    for a_idx, answer in enumerate(v):
        ...
        raw_rewards = torch.sum(raw_rewards, dim=1)
    ...
    print(idx, '-', ground_truth_answer, ',', selected)
    if selected == ground_truth_answer:
        correct += 1
