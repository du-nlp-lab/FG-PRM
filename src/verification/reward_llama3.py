...
from peft import LoraConfig, PeftModelForTokenClassification

from my_llama import LlamaForTokenClassification
...
class FactualityReward:
    def __init__(self,
        ...
        self.f_reward_model = PeftModelForTokenClassification.from_pretrained(model, model_path)
        ...
        self.sep = self.reward_tokenizer.eos_token
        
        # the id corresponds to the sep token
        self.sep_id = self.reward_tokenizer.eos_token_id
    ...
    def find_sep_position(self, input_ids):
        positions = torch.nonzero(input_ids == self.sep_id, as_tuple=False).squeeze().tolist()
