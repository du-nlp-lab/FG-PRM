...
#@add_start_docstrings(
#    """
#    Longformer Model with a token multi-classification head on top (a linear layer on top of the hidden-states output)
#    """,
#    LONGFORMER_START_DOCSTRING,
#)
class LongformerForTokenMultiClassification(LongformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
#        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
        self.classifiers = nn.ModuleList([nn.Linear(config.hidden_size, config.num_labels) for _ in range(self.num_classes)])
...
#        logits = self.classifier(sequence_output)
        ...
        for linear in self.classifiers:
            linear_output = linear(sequence_output)
            logits.append(linear_output)
        ...
        if labels is not None:
            loss_fct = CrossEntropyLoss()
#            loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            loss = loss_fct(logits.view(-1, self.num_labels, self.num_classes), labels.view(-1, self.num_classes))
